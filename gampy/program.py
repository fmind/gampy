        ...
        for s in steps:
            ...
            if len(s) == 1:
                ...
                raise DefinitionError(
                    "A step should contain 1, 2 or 3 arguments, not {} arguments.".format(
                        len(steps)
                    )
                )
            ...
            if not isinstance(step[0], Callable):
                raise DefinitionError(
                    "The 1st step argument should be a Callable, not a {}.".format(
                        type(step[0]).__name__
                    )
                )
            elif not isinstance(step[1], Iterable):
                raise DefinitionError(
                    "The 2nd step argument should be an Iterable, not a {}".format(
                        type(step[1]).__name__
                    )
                )
            elif not isinstance(step[2], Mapping):
                raise DefinitionError(
                    "The 3rd step argument should be Mapping, not a {}".format(
